...
//qRegisterMetaType<QList<QProgressBar> >("QList<QProgressBar>");
...
void Mainwindow::GetBindCatalog()
{
    ...
    if (isMacBinded)
    {
        watcher->addPath(localBindCatalog);
    }
...
void Mainwindow::ShowFileListTree()
{
    ...
    foreach (File file, Filelist)
    {
        ...
        QString isFile = file.isFile;
        QString updateTime = file.updateTime;
...
{
    if (QMessageBox::Yes ==
            QMessageBox::information(this, "Infomation", "Are you sure to change the binded catalog ?", QMessageBox::Yes|QMessageBox::No))
    {
        ui->infoTextBrowser->append("Choose a new catalog");
        ui->BindPushButton->setEnabled(false);
        ui->UnbindPushButton->setEnabled(false);
        ui->ChangeBindPushButton->setText("Okay");

//        if (ui->ChangeBindPushButton->)
    }
}
