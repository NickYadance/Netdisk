...
unsigned A,B,C,D,a,b,c,d,x[16];   //i临时变量,len文件长,flen[2]为64位二进制表示的文件初始长
...
void MD5_FILE(char * filename, char * res){
    ...
    FILE *fp;
    unsigned i,len,flen[2];
    if (filename[0]==34)
        filename[strlen(filename)-1]=0,strcpy(filename,filename+1);  //支持文件拖曳,但会多出双引号,这里是
    ...
    md5();
    fclose(fp);
    sprintf(res, "%08X%08X%08X%08X", PP(A), PP(B), PP(C), PP(D));
}
